...
def add_user(username, password):
    try:
        ...
        print(f"Error al agregar usuario: {error}")  # Este mensaje te dará más información sobre el error
        return False
    finally:
        if db:
            db.close()
...
def mostrar_recetas():
    try:
        db = get_db()
        cursor = db.cursor()
        # Seleccionar todas las recetas de la base de datos
        statement = "SELECT nombre, ingredientes, dificultad, calorias, tiempo_coccion FROM recetas"
        cursor.execute(statement)
        recetas = cursor.fetchall()

        # Convertir cada receta en un diccionario
        recetas_formateadas = []
...
def filtrar_recetas_por_ingredientes(ingredientes_disponibles):
    try:
        ...
        for receta in recetas:
            ...
            if all(ingrediente.strip() in ingredientes_disponibles for ingrediente in ingredientes_receta):
...
def obtener_detalles_ingrediente(nombre_ingrediente):
    app_id = "1cac8b2c"  # app_id
    app_key = "3b3ebb5648f51854d2c695d1c68174ca"  # app_key
