...
document.getElementById("filtrar-por-ingredientes").addEventListener("click", () => {
    ...
    axios.post('http://127.0.0.1:5000/recetas/filtrar', {
        ingredientes: ingredientesArray
    })
    .then(response => {
        const recetasFiltradas = response.data.recetas_filtradas;
        mostrarMisRecetas(recetasFiltradas);
    })
    .catch(error => {
        console.error("Error al filtrar recetas:", error);
        alert("Hubo un error al intentar filtrar las recetas.");
    });
});

document.getElementById("reiniciar-filtro").addEventListener("click", () => {
    mostrarMisRecetas();
    document.getElementById("ingredientes-filtrados").value = "";
});
